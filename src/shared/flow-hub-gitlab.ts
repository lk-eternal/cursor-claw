...
export class GitLabFlowHubClient {
  ...
  async readRawFile(projectId: number, filePath: string): Promise<string | null> {
    ...
    const res = await fetch(
      `${this.host}/api/v4/projects/${projectId}/repository/files/${enc}/raw?ref=main`,
  ...
  async commitFiles(
    ...
    const res = await fetch(`${this.host}/api/v4/projects/${projectId}/commits`, {
      method: "POST",
      headers: this.headers(),
  ...
  async fileExists(projectId: number, filePath: string): Promise<boolean> {
    const enc = encodeURIComponent(filePath)
    const res = await fetch(
      `${this.host}/api/v4/projects/${projectId}/repository/files/${enc}?ref=main`,
      { headers: this.headers() },
    )
    return res.ok
  }
