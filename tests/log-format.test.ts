...
describe("formatLogLineForUi", () => {
  it("replaces full sessionKey with resolveLabel", () => {
    const line = "[SDK] ch_e8f4e1be|oc_a9a2d8e20e851b4be59e5afb471804f3::project_90507a7006ce 已创建"
    const out = formatLogLineForUi(line, () => "📦 学员中心")
    expect(out).toBe("[SDK] 📦 学员中心 已创建")
  })
  ...
  it("replaces bracketed scheduled task session id", () => {
    ...
    const out = formatLogLineForUi(line, (sk) => (sk === id ? "⏰ 查询小班课数据情况" : undefined))
