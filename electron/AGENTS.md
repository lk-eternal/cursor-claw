...
- **SDK 上下文 footer（IM 回复）**：**单一落点** `agent-sdk` — `doFlushStreamPost(..., final=true)` 前 `applyContextFooterToBuffer` 写入 `streamBuffer`；中间 chunk 不含 footer。格式 `\n\n---\n上下文：{p}% ({usedK}k/{limitK}k)`（有上限）或 `\n\n---\n上下文：已用 {usedK}`（上限不可得）；usage 来自 `onDelta` `turn-ended.usage` **replace**（最后一轮快照，非 Run 内累加）；`used` 口径为 prompt 侧（input + cache 读写，不含 output）；上限来自 `Cursor.models.list` 查表或 modelId 启发式（session 级缓存）。`used <= 0` 时省略 footer。`appendContextFooter` 对已含「上下文：」的正文幂等。CLI `launchAgent` 路径不在 IM 回复 scope，daemon 侧不二次 append。
