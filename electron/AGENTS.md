...
- **SDK 错误 notify**：`streamRunEvents` catch、`status` ERROR/EXPIRED/CANCELLED、`run.status === "error"` 经 `notifySessionChat` 下发用户可理解文案；stack/tool 名仅写 UI 日志。用户主动 `stopSdkSession`（aborted）不 notify。有 usage 且上限可得时，错误 notify 正文末尾 optional append 上下文 footer。
...
- **SDK 上下文 footer（IM 回复）**：**单一落点** `agent-sdk` — `doFlushStreamPost(..., final=true)` 前 `applyContextFooterToBuffer` 写入 `streamBuffer`；中间 chunk 不含 footer。格式 `\n\n---\n上下文：{p}% ({usedK}k/{limitK}k)`；usage 来自 `onDelta` `turn-ended.usage` 累积；上限来自 `Cursor.models.list` 查表（session 级缓存）。limit 或 usage 不可得时**省略** footer（无 NaN）。`appendContextFooter` 对已含「上下文：」的正文幂等。CLI `launchAgent` 路径不在 IM 回复 scope，daemon 侧不二次 append。
