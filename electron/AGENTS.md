...
- **Presentation 出站（tool/thinking）**：`handleSdkEvent` 中 `tool_call` / `thinking` → `POST /api/presentation-event`（`PresentationEvent`）；工具 `running` 时清该 `tool_name` 的 `toolPresentationOutboundIds` 以新建 CardKit，完成/失败回传 `outbound_message_id` 供 PATCH；assistant 正文仍仅走 stream-text，不经 notify 发工具进度。
