...
- **SDK MCP 内联**：`mcp-sdk-loader.loadInlineMcpServers` 仅注入 stdio/command 型（补 `cwd`）；HTTP/url 型走 `settingSources`。`Agent.create` 与 resident 模式每次 `agent.send` 均须传入 `mcpServers`（SDK inline 不持久化）。有 usage 且上限可得时，错误 notify 正文末尾 optional append 上下文 footer。
...
- **SDK 上下文 footer（IM 回复）**：**单一落点** `agent-sdk` — `doFlushStreamPost(..., final=true)` 前 `applyContextFooterToBuffer` 写入 `streamBuffer`；中间 chunk 不含 footer。格式 `\n\n---\n上下文：{p}% ({usedK}k/{limitK}k)`（有上限）或 `\n\n---\n上下文：已用 {usedK}`（上限不可得）；usage 来自 `onDelta` `turn-ended.usage`，**Run 内取 peak、session 级 peak 跨 Run 保留**（末轮偏低或新 Run 仍可比）；`summary-completed` 清零 peak 后以压缩后用量为基准；`used` 口径为 prompt 侧（input + cache 读写，不含 output）；上限来自 `Cursor.models.list` 查表或 modelId 启发式（session 级缓存）。`used <= 0` 时省略 footer。`appendContextFooter` 对已含「上下文：」的正文幂等。CLI `launchAgent` 路径不在 IM 回复 scope，daemon 侧不二次 append。
