...
const ACTION_GUIDES: Record<string, string[]> = {
  ...
  "fix-bug": [
    ...
    "- MR 规则（评论里贴错 MR 等于误导测试，严禁）：",
    "  · 只用 project_get 查到的最近 submit-test 的 mrUrl（feature→测试分支），或本 feature 上尚未合入的同目标 MR",
    "  · 禁止贴已合入 release/生产基线的历史 MR，禁止把 feature→基线 / 已 merged 的 MR 当作本次修复凭证",
    "  · 尚无提测 MR 时：评论只写 feature 分支名 + commit，并提醒用户走「提测」节点开 MR；禁止自行开往生产基线的 MR",
    "- 在每个缺陷工作项下评论修复进度（@ 提缺陷人按提测节点的 mention 三步），评论必须写明：",
    "  · 修复点、关联 commit、本地验证方式",
    "  · 代码目前仅在 feature（及上述提测 MR 若有），尚未合入测试/release，请合入测试环境后再验证",
    "- 状态流转红线（差点出事的根因，必须遵守）：",
    "  · 代码只在 feature / 未合入测试分支时：禁止流转到「待验证」「已解决」「RESOLVED」等待验/关闭态，保持原处理中状态，只评论",
    "  · 仅当修复已合入测试分支（或用户明确说已部署到可复测环境）后，才可流转到待验证并 @ 提缺陷人",
    "  · 无权限或不确定当前该流转到哪一态时：只评论、不流转，并提醒用户手动处理",
    "- 产物写修复说明：每个缺陷的修复点、验证方式、MR/分支、是否已允许流转、遗留事项",
  ],
  "fill-release-doc": [
