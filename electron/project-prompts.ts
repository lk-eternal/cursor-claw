...
const ACTION_GUIDES: Record<string, string[]> = {
  ...
  "fix-bug": [
    ...
    "- 逐个缺陷：按分析报告定位根因 → 在 AI 工作目录内修复 → 本地验证（编译 / 测试 / 关键路径自测），禁止未验证就标记完成",
    ...
    "- 在每个缺陷工作项下评论修复说明：修复点、关联 commit、验证方式、MR 链接；@ 提缺陷人按提测节点的 mention 三步",
    "- 评论后流转缺陷工作项状态到待验证/已解决（无权限或流转失败时如实说明并提醒用户手动处理）",
  ...
  retest: [
    "复测。开发修复后会把缺陷流转到待验证并在缺陷下评论修复说明：",
    "- 从飞书项目拉取本项目关联的已解决/待验证缺陷列表（含开发的修复评论与 MR 信息）",
    "- 按原用例与缺陷复现路径逐项复测",
