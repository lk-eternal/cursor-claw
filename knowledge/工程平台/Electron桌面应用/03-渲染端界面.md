...
主工作目录（`workspaceDir`）与崩溃分析目录（`crashAnalysisDir`）：目录选择器 + autoSave；后者可「清除」恢复空值，留空显示「未配置（跳过归档）」，失败归档行为见 02 §七。
...
Settings「帮助引导」Tab 列出相同三步与飞书权限 JSON（`REQUIRED_FEISHU_SCOPES`）。
...
Daemon 启停、通道/队列/活跃 Agent、实时日志（5s 轮询 + 事件）；CLI 安装/登录入口。
...
开发模式 Tray/Updater 与打包版不同；Agent/Channel 细节见 `AgentPanel.tsx`、`ChannelPanel.tsx`。
