...
飞书/微信 `/` 指令：Daemon 写命令文件，Main 5s 轮询 claim 执行并回报。**飞书合并卡控制** MVP 经 HTTP/回复编辑（按钮回调 T8）。

## 二、设计决策与取舍

- **与 Agent 解耦**：`checkAndExecutePendingCommands` 在 `daemon-manager` 轮询中运行。
- **权限二分**：`isMainUser` 管理员；非管理员仅 `/status` `/stop`(本会话) `/reset` `/help`。
...
| 指令 | 权限 | 行为摘要 |
|------|------|----------|
| `/stop` | 全部 | 管理员停全部；他人停本会话 |
| `/status` | 全部 | Daemon/Agent/队列统计 |
| `/reset` | 全部 | 停会话 Agent + 清空 resume |
| `/help` | 全部 | 列出可用指令 |
| `/list` `/clean` `/restart` | 管理员 | 队列/重启 |
| `/workspace` `/model` `/mcp` | 管理员 | 配置 CRUD |
| `/task` `/workflow` `/chat` | 管理员 | 任务/工作流/临时会话 |
