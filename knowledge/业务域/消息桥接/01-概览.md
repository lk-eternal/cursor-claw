...
flowchart TD
  ...
  ENQ --> F1[F1 reply]
  ENQ --> PRE{≥2且未suppress?}
  PRE -->|是| MP[合并预览 debounce]
  PRE -->|否| SKIP[仅 F1]
  IDLE[idle补偿] --> MP
  MP --> ENS[instant poll ensure]
  SKIP --> ENS
  MOD --> ENS
  ENS --> POLL[poll claim]
  POLL --> AG[Agent]
...
- 预览仅飞书私聊 SDK；≥2 条待领取；processing/claimed/流式时抑制；idle 后须补偿调度；instant poll claim 前须 ensure 预览 outbound。
...
- 预览 debounce 500ms；phase 丢失 F1 靠 `.claimed` 兜底；idle 钩子 `scheduleMergePreviewIfEligible`；instant poll `ensureMergePreviewSentBeforeClaim`。
...
2026-06-27 T-FIX-01：idle 补偿 + instant poll 预览守卫（archive 20260627150041）。
