...
- **idle 补偿**：processing 期间被抑制的 debounce 不会自动重跑；phase 转 `idle` 后 daemon `scheduleMergePreviewIfEligible` 补调度（T-FIX-01）。
- **instant poll 时序**：SDK `wait=false` poll 须先 `ensureMergePreviewSentBeforeClaim` 再 claim，避免「未预览先领整批」（T-FIX-01）。
...
- **F2 预览**：header `【合并预览 · ID：MG-{profile}-{ts}】` + `【消息 N】` 正文 + 引导「回复本条消息…」；批次 MG-id 不变，更新加「· 已更新」；registry 登记全部 outbound id（含分条续页）。Agent 领取前须至少一次 outbound 可见（push 路径 debounce + idle 补偿 + instant poll ensure 三路径保障）。
...
2026-06-27 T-FIX-01：idle 补偿 + instant poll 预览窗口守卫（archive 20260627150041）。
