...
- **F1 入队确认**：`buildEnqueueStatusText` — starting/processing/idle×pending 组合文案（见 02）；pending>1 附加排队数。
...
- **预览时序（T-FIX-01）**：phase 转 idle 后 `scheduleMergePreviewIfEligible` 补 F4 跳过的 debounce；`wait=false` poll claim 前 `await ensureMergePreviewSentBeforeClaim`；suppress 时 no-op 仍 claim。
- **停止**：`ackOnReply`、`stop_progress:true`、stream `final`。
- **流式**：`isStreamTextEligible` 主用户 p2p；CardKit 路径见 [02-飞书通道.md](./02-飞书通道.md)。
...
| `scheduleMergePreviewIfEligible` / `ensureMergePreviewSentBeforeClaim` | idle 补偿 / instant poll 预览守卫 |
...
- 合并预览/F3 仅飞书私聊。
- `blocking=true` poll 无预览守卫（主用户 SDK 走 instant）。

## 十、变更记录

2026-06-27：F1/unclaimed/phase/merge override + T-FIX-01 预览时序（archive 20260627150041）。
