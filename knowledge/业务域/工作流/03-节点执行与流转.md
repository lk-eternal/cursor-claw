...
独立 Agent sessionKey：`{chatId}::wf_{instanceId}_{nodeId}`。

## 六、数据

仅改实例 JSON：context、currentNodeId、status、nodeHistory、stepCount。
...
- isolated 后沿用新 Agent；`instance.sessionKey` 未持久化。
- 驳回 Prompt 依赖 context，未单独注入上次 output。
- resume/recover 无产品入口。
