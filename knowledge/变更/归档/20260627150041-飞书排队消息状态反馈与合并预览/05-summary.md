...
- **daemon**：入队 F1 文案、合并预览状态机、F3 拦截、phase API、poll override 与清理；**T-FIX-01** idle 补偿 + instant poll 预览守卫。
...
- [x] `knowledge/业务域/消息桥接/04-消息队列与路由.md` — F1 阶段文案、unclaimed 计数、phase API、merge override 与 poll 清理；**T-FIX-01** idle 补偿 + instant poll 守卫
- [x] `knowledge/业务域/消息桥接/02-飞书通道.md` — 合并预览格式、MG-id、F3 回复修改、F4 抑制规则；**T-FIX-01** 时序与 ensure 路径
