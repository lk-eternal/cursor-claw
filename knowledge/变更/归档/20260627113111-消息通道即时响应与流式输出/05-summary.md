...
**Rev1**：飞书 SDK 流式由「PATCH 更新 text 消息首选」改为「CardKit 流式卡片首选」，与 `02-design` Rev1 对齐；PATCH/分段保留为 CardKit 失败 fallback。04-review 通过；`08-verify-issue` 第 1 轮打回场景（飞书+SDK 主用户私聊多条半幅递增）经 Rev1 修复后**联调复验通过**（2026-06-27）。

**T-FIX-04**：第 2 轮打回（`.claimed` 滞留）由 lite 变更 `20260627223927-SDK流式final队列ack`（**1.7.2**）闭合，**联调复验通过**（2026-06-27）。
...
| 位置 | 注释摘要 | 升级路径 |
|------|----------|----------|
| ~~`electron/agent-sdk.ts`（04-review §7）~~ **已闭合（1.7.2 lite）** | ~~SDK `stream-text final` 未传 inbound `message_id`；`.claimed` 依赖后续 MCP `send_text`~~ → lite `20260627223927-SDK流式final队列ack` 传 inbound id 闭环 ack | T-FIX-04 已实施；verify-issue 第 2 轮复验通过 |
...
- [x] `knowledge/业务域/消息桥接/02-飞书通道.md` — Get 时序、流式 PATCH/分段降级；Rev1 CardKit 流式卡片（create/send/update/close、`card_id`/`sequence`、fallback）— 待 librarian 落盘，archive 前勾选
- [x] `knowledge/业务域/消息桥接/03-微信通道.md` — typing 生命周期与 sendText 解耦
- [x] `knowledge/业务域/消息桥接/01-概览.md` — 三态主流程 mermaid、术语与全局约束；Rev1 CardKit 术语 — 待 librarian 落盘，archive 前勾选
- [x] `knowledge/业务域/消息桥接/04-消息队列与路由.md` — Rev1 CardKit 与 stream-text 状态字段术语 — 待 librarian 落盘，archive 前勾选
